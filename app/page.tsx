...
import { useAddNewUserMutation, useChangeStatusMutation, useDeleteImgTodosMutation, useDeleteTodosMutation, useEditUserMutation, useGetTodosQuery } from "./api/todo"
                ...
                {e.images?.map((img: IImage) => (
                  <div key={img.id}>
                    <img
                      ...
                      alt=""
                    />
                    <Button className="my-3" onClick={() => refetch(deleteImage(img.id))} >
                      DlImg
                    </Button>
                  </div>
                ))}
              </div>
              <Button className="m-3" type="primary">Add Img</Button>
              <div className="p-4">
                ...
                <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
                  <div className="flex gap-2">
                    <button
                      onClick={() => refetch(deleteUser(e.id))}
                  ...
                  <input onChange={() => refetch(changeSatus(e.id))} type="checkbox" checked={e.isCompleted} className="w-4 h-4 accent-green-600 cursor-pointer" />
                </div>
              </div>
